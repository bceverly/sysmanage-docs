...
INTRO_BY_LOCALE = {
    ...
    "fr": "La page Paramètres → Mise en miroir des dépôts est présentée comme une barre d'onglets avec un onglet par gestionnaire de paquets. Aujourd'hui les quatre onglets sont <strong>Ubuntu/Debian</strong> (apt), <strong>RHEL/Fedora</strong> (dnf -- couvre RHEL, Fedora, Oracle Linux, Rocky, Alma, CentOS Stream), <strong>openSUSE/SLES</strong> (zypper) et <strong>FreeBSD</strong> (pkg). Chaque onglet représente un hôte miroir exécutant un PM ; la boîte de dialog Ajouter un miroir hérite donc de son gestionnaire de paquets de l'onglet actif.",
